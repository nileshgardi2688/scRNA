...
#####Unbiased
setwd("/home/pavan/Nilesh/single_cell_data/Gynecological_cancers/GSE173682/")
...
for(x in dirs){
    ...
    cts <- ReadMtx(mtx = paste0('Data/',x,'/matrix.mtx'),
                   features = paste0('Data/',x,'/features.tsv'),
                   cells = paste0('Data/',x,'/barcodes.tsv'))
    
    # create seurat objects
    assign(x, CreateSeuratObject(counts = cts,,min.cells = 3))
}
...
# merge datasets
ls()
merged_seurat <- merge(GSM5276933, y = c(GSM5276934, GSM5276935, GSM5276936, GSM5276937, GSM5276938, GSM5276939, GSM5276940, GSM5276941, GSM5276942, GSM5276942),
      add.cell.ids = ls()[3:13],
      project = 'Gynec')
...
merged_seurat_filtered <- subset(merged_seurat, subset = nCount_RNA > 400 & nFeature_RNA > 100 & nFeature_RNA < 8000 & mitoPercent < 10)
...
DimPlot(merged_seurat_filtered, reduction = "umap.unintegrated",group.by ="Patient",label=TRUE)
...
Idents(merged_seurat_filtered) <- "RNA_snn_res.0.3"
...
Idents(merged_seurat_filtered) <- "RNA_snn_res.0.3"
markers.to.plot <- c("MLANA","MITF","DCT","PRAME","CD3D","CD3E","CD8A","CD19","MS4A1","IGHG1","MZB1","SDC1","CD79A","CD68","CD163","CD14","FGFBP2","FCG3RA","CX3CR1","RPE65","RCVRN","FGF7","PECAM1","VWF","DLL3","TYRP1")
png("Dotplot_clusterd_known_markers.png",width=1400,height=700)
...
####################################################Violin plots
#Tumor
png("Tumor.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("MITF","MLANA","DCT"),layer = "data", log = TRUE)
dev.off()

#Tumor
png("Tumor_PRAME.png")
VlnPlot(merged_seurat_filtered, features = c("PRAME"),layer = "data", log = TRUE)
dev.off()

#T
png("T_cell.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("CD3D","CD3E","CD8A"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()

#B
png("B_cell.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("CD19","CD79A","MS4A1"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()

#Plasma_cells
png("Plasma_cell.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("IGHG1","MZB1","SDC1","CD79A"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()

#Monocytes and macrophages
png("Monocytes_macrophages.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("CD68","CD163","CD14"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()

#NK cells
png("NK_cells.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("FGFBP2","FCG3RA","CX3CR1"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()

#Retinal pigment epithelium (RPE65),
png("Retinal_pigment_epithelium.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("RPE65"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()

#Photoreceptor cells (RPE65),
png("Photoreceptor.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("RCVRN),"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()

#Fibroblast
png("Fibroblast.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("FGF7"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()


#Endothelial cells
png("Endothelial_cells.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("PECAM1","VWF"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()

#DLL3 TYRP1
png("DLL3_TYRP1.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("DLL3","TYRP1"),layer = "data", log = TRUE)+ RotatedAxis()
dev.off()

png("House_keeping_genes.png",width=1400,height=700)
VlnPlot(merged_seurat_filtered, features = c("GAPDH","ACTB","B2M"),layer = "data", log = TRUE)
dev.off()
...
write.table(FetchData(object = merged_seurat_filtered, vars = c("MLANA","MITF","DCT","PRAME","CD3D","CD3E","CD8A","CD19","CD79A","MS4A1","IGHG1","MZB1","SDC1","CD79A","CD68","CD163","CD14","FGFBP2","FCG3RA","CX3CR1","RPE65","RCVRN","FGF7","PECAM1","VWF","DLL3","TYRP1"), layer = "data"),"After_integration_All_known_markers_DLL3_TYRP1_data.txt",sep="\t")
